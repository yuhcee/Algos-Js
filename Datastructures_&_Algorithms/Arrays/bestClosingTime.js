 ...
 * at the `ith` hour
 * - whereas `'N'` indicates that no customers come at the ``ith``
 * hour.
 * If the shop closes at the `jth` hour `(0 <= j <= n)`, the
 ...
 * a **minimum** penalty.
 *
 * **Note** that if a shop closes at the `jth` hour, it means the
